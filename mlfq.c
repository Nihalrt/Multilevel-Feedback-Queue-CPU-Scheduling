...
void handle_instruction(Instruction_t *instruction, int tick) {
	...
	if(instruction->burst_time == 0) { 
		...
		// Task Termination
        Task_t *terminated_task = &task_table[task_id];
        int waiting_time = terminated_task->total_wait_time;
        int turn_around_time = waiting_time + terminated_task->total_execution_time;
		printf("[%05d] id=%04d EXIT wt=%d tat=%d\n", tick, task_id, 
			waiting_time, turn_around_time);
        task_table[task_id].id = 0;          
        task_table[task_id].next = NULL; 
	} else {
        
        Task_t *bursting_task;
        bursting_task = &task_table[task_id];
         if(bursting_task->current_queue==0){
            bursting_task->current_queue=1;
        }
        bursting_task->burst_time = instruction->burst_time;
        bursting_task->remaining_burst_time = instruction->burst_time;
        enqueue(get_queue_by_id(bursting_task->current_queue), bursting_task);   
      }
	}
...
void boost(int tick) {
	...
	if (tick % BOOST_INTERVAL != 0){
     ...
     if(current_task!=NULL){
        ...
        count = remaining_quantum;
        if(count > 2){
            remaining_quantum = 2;
        }
        else{
            remaining_quantum = count;
        }        
    }
...
void scheduler() {
    ...
    if(current_task!=NULL && current_task->remaining_burst_time > 0 && remaining_quantum==0){
        decrease_task_level(current_task);
        enqueue(get_queue_by_id(current_task->current_queue), current_task);
        current_task = dequeue(get_queue_by_id(current_task->current_queue));
        int d = current_task->current_queue;
        remaining_quantum = QUEUE_TIME_QUANTUMS[d-1];
    }
    if(priority_task!=NULL){
        if(current_task==NULL){
          current_task = dequeue(get_queue_by_id(priority_task->current_queue));
            int f = current_task->current_queue;
            remaining_quantum = QUEUE_TIME_QUANTUMS[f-1];  
        }
        else if (priority_task->current_queue < current_task->current_queue){
            enqueue(get_queue_by_id(current_task->current_queue), current_task);
           current_task = dequeue(get_queue_by_id(priority_task->current_queue)); 
            int g = current_task->current_queue;
            remaining_quantum = QUEUE_TIME_QUANTUMS[g-1];
        }
    }
}
/*
...
void update_task_metrics() {
    ...
    while (task != NULL) {
        ...
        task = task->next;  // Move to the next task in the queue
    }
    task = queue_2->start;
    while (task != NULL) {
        task->total_wait_time++;
